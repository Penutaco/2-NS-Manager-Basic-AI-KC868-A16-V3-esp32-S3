    ...
    with open(input_file, 'r', newline='') as csvfile:
        ...
        for row in reader:
            ...
            if len(row) > event_info_col and row[event_info_col]:  # Check if Event Info column has data
                ...
                # NEW: Check if this line has sensor data too
                has_sensor_data = False
                if len(row) > ph_val_col and row[time_col]:
                    try:
                        # Try to extract EC and pH values to check if they exist
                ...
                # If no sensor data on this line, set flag to look ahead
                if not has_sensor_data:
                    # Store current event info for pairing with future sensor data
                    if current_cycle_num is not None and current_event_type is not None:
                        row_timestamp = row[time_col] if time_col < len(row) and row[time_col] else ""
                        # Add basic event row
                        extracted_data.append([
                        ...
                        # If there's a meaningful action, start looking ahead for sensor data
                        if current_action:
                    ...
                    # This line has both event info AND sensor data
                    print(f"📍 Found event info and sensor data on the same line")
            
            # Process sensor data row - updated to handle look-ahead logic
            elif len(row) > ph_val_col and row[time_col]:
                try:
                    ...
                    if looking_for_data:
                        ...
                        # Stop looking if we've gone past our limit
                        if look_ahead_count >= look_ahead_limit:
                            print(f"❌ Reached look-ahead limit without finding sensor data")
                            looking_for_data = False
                            pending_event_data = None
                    
                    # Original measurement detection - only run if not in look-ahead mode
                    elif measure_pending and current_event_type and current_cycle_num:
                        if ph_val != 0.0 or ph_v != 0.0:
                            timestamp = row[time_col]
                            
                            # Original code for adding measurement data
                            extracted_data.append([
                                current_cycle_num,     # Cycle Number
                                current_event_type,    # Event Type
                                current_event_num,     # Event
...
def process_all_files(data_dir):
    ...
    for sensor_file in sensor_files:
        ...
        output_file = os.path.join(data_dir, f"event_cycle_data_{timestamp}.csv")
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        if input_file:
            ...
            output_file = os.path.join(input_dir, base_name.replace("sensor_data", "event_cycle_data"))
