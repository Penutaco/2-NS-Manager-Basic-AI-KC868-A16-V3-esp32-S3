    ...
    with open(input_file, 'r', newline='') as csvfile:
        ...
        for row in reader:
            ...
            if len(row) > event_info_col and row[event_info_col]:  # Check if Event Info column has data
                ...
                cycle_match = re.search(r'(?:Starting new|Found|Detected) PRG(\w+) cycle #?(\d+)(?: of (\d+))?', event_info)
                if cycle_match:
                    prg_type = cycle_match.group(1)
                    current_cycle_num = int(cycle_match.group(2))
                    total_cycles = int(cycle_match.group(3)) if cycle_match.group(3) else None
                    current_event_type = f"PRG{prg_type}"
                    print(f"👉 Found {current_event_type} cycle #{current_cycle_num}" + 
                          (f" of {total_cycles}" if total_cycles else ""))
