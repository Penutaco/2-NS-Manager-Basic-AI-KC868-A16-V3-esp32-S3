...
def extract_cycle_data(input_file, output_file):
    ...
    with open(input_file, 'r', newline='') as csvfile:
        ...
        for row in reader:
            ...
            if len(row) >= 11 and row[10]:  # Check if Event Info column has data
                ...
                if "Recorded pH calibration point" in event_info or "PRGSample pH measurement" in event_info:
                    measure_pending = True
            ...
            if len(row) >= 9 and row[0] and not has_event_info:  # Has timestamp and numeric data
                try:
                    ...
                    if measure_pending and current_event_type and current_cycle_num:
                        ...
                        if ph_val != 0.0 or ph_v != 0.0:
                            ...
                            print(f"✅ Found sensor data for {current_event_type} cycle {current_cycle_num}")
                            print(f"   EC: {ec_v}V / {ec_ppm}ppm, pH: {ph_v}V / {ph_val}")
                            
                            # Reset the pending flag after capturing data
                            measure_pending = False
    ...
    if extracted_data:
        with open(output_file, 'w', newline='') as csvfile:
            ...
            # Write header
            writer.writerow([
                'Event Type', 'Cycle Number', 'Event Time',
        ...
        print(f"✅ Successfully wrote {len(extracted_data)} cycle data entries to {output_file}")
        
        # Open output folder in Finder (macOS)
        os.system(f"open -R '{output_file}'")
        print(f"📂 Opened folder containing the output file")
    else:
