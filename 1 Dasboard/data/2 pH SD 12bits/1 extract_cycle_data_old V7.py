...
def extract_cycle_data(input_file, output_file):
    ...
    if extracted_data:
        with open(output_file, 'w', newline='') as csvfile:
            ...
            # Organizar dados em ciclos
            data_by_type_cycle = {}
            for row in extracted_data:
                event_type = row[0]
                cycle_num = row[1]
                
                # Criar chave para agrupar corretamente
                if event_type == "PRGCal":
                    group_key = f"{event_type}_{cycle_num}"
                else: # PRGSample
                    # Encontrar qual é o ciclo PRGCal atual
                    cal_cycle = 1  # Default é 1
                    for i in range(len(extracted_data)-1, -1, -1):
                        if extracted_data[i][0] == "PRGCal" and i < extracted_data.index(row):
                            cal_cycle = extracted_data[i][1]
                            break
                    group_key = f"PRGCal_{cal_cycle}_Sample"
                
                if group_key not in data_by_type_cycle:
                    data_by_type_cycle[group_key] = []
                data_by_type_cycle[group_key].append(row)
            
            # Determinar a estrutura de colunas
            column_names = ['Event Type', 'Cycle Number', 'Time', 'EC (V)', 'EC (ppm)', 
                            'pH (V)', 'pH Value', 'Nitrate (V)', 'Nitrate (ppm)']
            header_row = []
            
            # Criar o cabeçalho com colunas repetidas
            group_keys_ordered = sorted(data_by_type_cycle.keys())
            ...
            max_rows = 0
            for group_data in data_by_type_cycle.values():
                max_rows = max(max_rows, len(group_data))
            
            # Escrever os dados em colunas adjacentes, organizados por tipo e ciclo
            for i in range(max_rows):
                data_row = []
                
                for key in group_keys_ordered:
                    group_data = data_by_type_cycle[key]
                    
                    if i < len(group_data):
                        # Adicionar dados deste grupo
                        data_row.extend(group_data[i])
                    else:
                        # Preencher com valores vazios se não houver mais dados
