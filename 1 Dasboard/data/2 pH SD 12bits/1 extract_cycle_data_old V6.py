...
def extract_cycle_data(input_file, output_file):
    ...
    if extracted_data:
        with open(output_file, 'w', newline='') as csvfile:
            ...
            # Organizar dados por tipo e ciclo
            cal_samples = []
            sample_samples = []
            current_type = None
            ...
            for row in extracted_data:
                event_type = row[0]
                # Quando mudamos de PRGCal para PRGSample ou vice-versa, começamos um novo grupo
                if event_type != current_type:
                    current_type = event_type
                    if event_type == "PRGCal":
                        cal_samples.append([])
                    else:
                        sample_samples.append([])
                
                # Adicionar ao grupo atual
                if event_type == "PRGCal":
                    cal_samples[-1].append(row)
                else:
                    sample_samples[-1].append(row)
            
            # Criar o cabeçalho repetido para cada conjunto
            header_row = []
            column_names = ['Event Type', 'Cycle Number', 'Time', 'EC (V)', 'EC (ppm)', 'pH (V)', 'pH Value', 'Nitrate (V)', 'Nitrate (ppm)']
            
            # Determinar quantos conjuntos temos (combinações de cal/sample)
            max_sets = max(len(cal_samples), len(sample_samples))
            ...
            # Determinar o número máximo de linhas em qualquer grupo
            max_rows = 0
            for group in cal_samples:
                max_rows = max(max_rows, len(group))
            for group in sample_samples:
                max_rows = max(max_rows, len(group))
            
            # Escrever os dados em colunas adjacentes
            for i in range(max_rows):
                data_row = []
                
                # Intercalar grupos de PRGCal e PRGSample
                for j in range(max_sets):
                    # Adicionar dados de PRGCal se disponíveis
                    if j < len(cal_samples) and i < len(cal_samples[j]):
                        data_row.extend(cal_samples[j][i])
                    else:
                        # Preencher com valores vazios se não houver dados
                        data_row.extend([''] * len(column_names))
                    
                    # Adicionar dados de PRGSample se disponíveis
                    if j < len(sample_samples) and i < len(sample_samples[j]):
                        data_row.extend(sample_samples[j][i])
                    else:
                        # Preencher com valores vazios se não houver dados
                        data_row.extend([''] * len(column_names))
        ...
        print(f"✅ Successfully wrote grouped cycle data to {output_file}")
    else:
