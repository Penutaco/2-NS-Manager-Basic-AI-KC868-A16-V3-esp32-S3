...
def read_data_from_csv():
    """Read data from CSV files for visualization with support for rotated files"""
    global global_data, event_data
    
    csv_file_position = {}  # Track position for each file
    current_csv_file = None
    last_full_reload = 0
    last_rotation_check = 0
    
    try:
        print("🔍 Starting CSV monitor for visualization with rotation support...")
        
        while True:
            try:
                current_time = time.time()
                
                # Check for file rotation every 30 seconds
                if current_time - last_rotation_check > 30:
                    ...
                    if data_files:
                        ...
                        # If new file detected, reload all historical data
                        if current_csv_file != latest_path:
                            current_csv_file = latest_path
                            if latest_path not in csv_file_position:
                                csv_file_position[latest_path] = 0
                            last_full_reload = time.time()
                            print(f"📊 New CSV file detected: {latest_file}")
                            print("🔄 Reloading all historical data from rotated files...")
                            reload_all_historical_data()
                ...
                if current_time - last_full_reload > 300:  # Every 5 minutes
                    ...
                    reload_all_historical_data_for_window(max_window)
                
                
                # Read new lines from current CSV file
                if current_csv_file and os.path.exists(current_csv_file):
                    current_position = csv_file_position.get(current_csv_file, 0)
                    ...
                    with open(current_csv_file, 'r') as f:
                        ...
                        csv_file_position[current_csv_file] = f.tell()
...
def reload_all_historical_data():
    """Reload all historical data from all rotated CSV files"""
    try:
        # Find all sensor data CSV files
        data_files = [f for f in os.listdir(DATA_DIR) if f.startswith('sensor_data_') and f.endswith('.csv')]
        data_files.sort()  # Sort chronologically
        
        print(f"🔄 Loading historical data from {len(data_files)} CSV files...")
                            ...
                            try:
                                ...
                                if time_ms:
                                    ...
                                    if len(row) > 11 and row[11].strip():
                                        temp_events.append(row[11].strip())
        ...
        with lock:
            ...
            global_data.extend(temp_data)
            event_data.clear()
            event_data.extend(temp_events[-1000:])  # Keep last 1000 events
            
        print(f"📈 Loaded {len(global_data)} total data points from all CSV files")
        
    except Exception as e:
        print(f"❌ Error reloading all historical data: {e}")

def reload_all_historical_data_for_window(time_window_seconds):
    """Reload historical data from multiple files to support current time window"""
    try:
        current_time_ms = time.time() * 1000
        cutoff_time_ms = current_time_ms - (time_window_seconds * 1000)
        ...
        with lock:
            # Check if we need more historical data
            if not global_data or global_data[0][0] > cutoff_time_ms:
                print(f"🔄 Reloading data for {time_window_seconds}s time window from all files...")
                ...
                # Sort by timestamp and update global data
                temp_data.sort(key=lambda x: x[0])
                global_data.clear()
                global_data.extend(temp_data)
                print(f"📈 Loaded {len(global_data)} data points for {time_window_seconds}s window from all files")
                
    except Exception as e:
        print(f"❌ Error reloading data for time window: {e}")
...
def update_graphs(n_intervals, time_window, viz_mode, *axis_controls):
    ...
    # Status message with multi-file information
    latest_time = time_sec[-1] if time_sec else 0
    earliest_time = time_sec[0] if time_sec else 0
    mode_text = "Window" if viz_mode == 'window' else "Full"
    
    # Count available CSV files and calculate time span
    try:
        data_files = [f for f in os.listdir(DATA_DIR) if f.startswith('sensor_data_') and f.endswith('.csv')]
        file_count = len(data_files)
        
        if len(time_sec) > 1:
            time_span_hours = (latest_time - earliest_time) / 3600000  # Convert ms to hours
            if file_count > 1:
                status = f"📊 {len(data)} points from {file_count} files | Span: {time_span_hours:.1f}h | Latest: {latest_time:.0f} ms | Mode: {mode_text}"
            else:
                status = f"📊 {len(data)} points | Span: {time_span_hours:.1f}h | Latest: {latest_time:.0f} ms | Mode: {mode_text}"
        else:
            if file_count > 1:
                status = f"📊 {len(data)} points from {file_count} files | Latest: {latest_time:.0f} ms | Mode: {mode_text}"
            else:
...
def main():
    """Main function to run visualization with multi-file rotation support"""
    print("=== Hydroponic Sensor Visualization (Multi-File Support) ===")
    print(f"Data Directory: {DATA_DIR}")
    print("Features:")
    print("  🔄 File rotation support - reads from multiple CSV files")
    print("  📈 Historical data continuity across rotations") 
    print("  ⏰ Long-term time window support (up to 8+ hours)")
    print("="*60)
    
    try:
        # Start CSV monitoring with rotation support
        csv_thread = start_csv_monitor_thread()
        
        print("\n🚀 Visualization system started successfully!")
        print("📊 Multi-file CSV monitoring active")
        print("🔄 File rotation detection enabled")
