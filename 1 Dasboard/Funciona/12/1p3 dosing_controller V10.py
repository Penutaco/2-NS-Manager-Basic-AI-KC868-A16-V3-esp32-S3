...
CONFIG = {
    "tank_volume_liters": 3.0,
    "target_ph": 6.0,                     # DEPRECATED - use stages
    "target_ec": 1.0,                     # DEPRECATED - use stages  
    "ph_tolerance": 0.2,                  # DEPRECATED - use stages
    "ec_tolerance": 0.1,                  # DEPRECATED - use stages
...
class DosingController:
    ...
    def chemistry_based_dosing_decision(self, measurement_type):
        """Make dosing decisions based on current stage configuration"""
        ...
        if stage_config is None:
            logger.info("Program ended - no more enabled stages")
            self.running = False
            return None
        
        current_stage = self.config["current_stage"]
        logger.info(f"Operating in Stage {current_stage}")
        ...
        ph_deviation = abs(ph_val - stage_config["target_ph"])
        ec_deviation = abs(ec_mScm - stage_config["target_ec"])
        
        logger.info(f"Stage {current_stage} analysis - pH: {ph_val:.2f} (target: {stage_config['target_ph']}, deviation: {ph_deviation:.2f}), EC: {ec_mScm:.2f} mS/cm (target: {stage_config['target_ec']}, deviation: {ec_deviation:.2f})")
        
        # Check EC continuous monitoring setting
        ec_needs_dosing = False
        if stage_config["ec_continuous_monitoring"]:
            # Continuous monitoring - dose if outside tolerance
            if ec_deviation > stage_config["ec_tolerance"] and ec_mScm < stage_config["target_ec"]:
                ec_needs_dosing = True
        else:
            # One-time adjustment - check if EC was already adjusted in this stage
            stage_ec_dosing_count = len([e for e in self.dosing_history 
                                       if e.get('pump') == 'ec_plus' and 
                                       e.get('stage', current_stage) == current_stage])
            
            if stage_ec_dosing_count == 0 and ec_mScm < stage_config["target_ec"]:
                ec_needs_dosing = True
                logger.info(f"Stage {current_stage}: One-time EC adjustment needed")
        
        # First priority: Check if EC needs correction
        if ec_needs_dosing:
            # Temporarily update config with stage values for calculation
            original_target_ec = self.config.get('target_ec')
            self.config['target_ec'] = stage_config["target_ec"]
            
            _, volume_needed = self.calculate_chemical_dosing_volume("ec_plus")
            
            # Restore original config
            if original_target_ec is not None:
                self.config['target_ec'] = original_target_ec
            
            # Apply dosing count logic
            ...
            if ec_dosing_count == 0:
                ...
                volume_needed /= 1
                logger.info(f"Subsequent EC+ dosing: dividing volume by 1: {volume_needed:.2f} mL")
            ...
            if dosing_time >= self.config['min_dosing_time_ms']:
                logger.info(f"Stage {current_stage} Chemistry decision: Increase EC with {volume_needed:.2f} mL ({dosing_time} ms)")
                return "ec_plus", dosing_time, volume_needed
        
        # Second priority: Check if pH needs correction
        if ph_deviation > stage_config["ph_tolerance"]:
            # Temporarily update config with stage values for calculation
            original_target_ph = self.config.get('target_ph')
            self.config['target_ph'] = stage_config["target_ph"]
            
            if ph_val < stage_config["target_ph"]:
                _, volume_needed = self.calculate_chemical_dosing_volume("ph_plus")
                pump_type = "ph_plus"
                dosing_count_key = 'ph_plus'
            else:
                _, volume_needed = self.calculate_chemical_dosing_volume("ph_minus")
                pump_type = "ph_minus"
                dosing_count_key = 'ph_minus'
            
            # Restore original config
            if original_target_ph is not None:
                self.config['target_ph'] = original_target_ph
            
            # Apply dosing count logic
            ph_dosing_count = len([e for e in self.dosing_history if e.get('pump') == dosing_count_key])
            
            if ph_dosing_count == 0:
                volume_needed /= 10
                logger.info(f"First {pump_type} dosing: dividing volume by 10: {volume_needed:.2f} mL")
            else:
                volume_needed /= 1
                logger.info(f"Subsequent {pump_type} dosing: dividing volume by 1: {volume_needed:.2f} mL")
            ...
            if dosing_time >= self.config['min_dosing_time_ms']:
                logger.info(f"Stage {current_stage} Chemistry decision: {pump_type} with {volume_needed:.2f} mL ({dosing_time} ms)")
                return pump_type, dosing_time, volume_needed
        
        logger.info(f"Stage {current_stage} Chemistry decision: No dosing needed, parameters within tolerance")
        return None

    def get_current_stage_config(self):
        """Get the configuration for the current active stage"""
        current_stage = self.config["current_stage"]
        
        # Check if current stage is enabled
        if not self.config.get(f"stage_{current_stage}_enabled", False):
            logger.warning(f"Stage {current_stage} is disabled, ending program")
            return None
        
        return {
            "target_ph": self.config[f"stage_{current_stage}_target_ph"],
            "ph_tolerance": self.config[f"stage_{current_stage}_ph_tolerance"],
            "target_ec": self.config[f"stage_{current_stage}_target_ec"],
            "ec_tolerance": self.config[f"stage_{current_stage}_ec_tolerance"],
            "ec_continuous_monitoring": self.config[f"stage_{current_stage}_ec_continuous_monitoring"],
            "ratio_sol_a_b_c": self.config[f"stage_{current_stage}_ratio_sol_a_b_c"]
        }
    ...
    def check_stage_transition(self):
        ...
        current_time = time.time()
        hours_elapsed = (current_time - self.config["program_start_time"]) / 3600
        current_stage = self.config["current_stage"]
        
        # Check if we should transition to next stage
        for stage in range(current_stage + 1, 15):  # Check stages current+1 to 14
            if not self.config.get(f"stage_{stage}_enabled", False):
                continue  # Skip disabled stages
                
            trigger_hours = self.config[f"stage_{stage}_trigger_hours"]
            if hours_elapsed >= trigger_hours:
                logger.info(f"STAGE TRANSITION: Moving from Stage {current_stage} to Stage {stage} after {hours_elapsed:.1f} hours")
