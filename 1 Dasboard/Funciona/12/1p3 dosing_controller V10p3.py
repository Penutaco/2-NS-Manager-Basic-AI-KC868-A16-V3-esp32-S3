...
CONFIG = {
    ...
    "serial_port": "/dev/ttyUSB0",  # Porta serial do ESP32
    ...
    "stage_2_trigger_hours": 24.0,
    "stage_2_target_ph": 5.8,
    "stage_2_ph_tolerance": 0.2,
    "stage_2_target_ec": 1.2,
    ...
    "stage_3_trigger_hours": 48.0,
    "stage_3_target_ph": 5.8,
    "stage_3_ph_tolerance": 0.2,
    "stage_3_target_ec": 1.4,
    ...
    "stage_4_trigger_hours": 72.0,
    "stage_4_target_ph": 6.0,
    "stage_4_ph_tolerance": 0.2,
    "stage_4_target_ec": 1.6,
    ...
    "stage_8_trigger_hours": 168.0,
    "stage_8_target_ph": 6.0,
    "stage_8_ph_tolerance": 0.2,
    "stage_8_target_ec": 2.4,
    ...
    "stage_9_trigger_hours": 192.0,
    "stage_9_target_ph": 6.0,
    "stage_9_ph_tolerance": 0.2,
    "stage_9_target_ec": 2.6,
    ...
    "stage_10_trigger_hours": 216.0,
    "stage_10_target_ph": 5.8,
    "stage_10_ph_tolerance": 0.2,
    "stage_10_target_ec": 2.8,
    ...
    "stage_11_trigger_hours": 240.0,
    "stage_11_target_ph": 5.8,
    "stage_11_ph_tolerance": 0.2,
    "stage_11_target_ec": 3.0,
    ...
    "stage_12_trigger_hours": 264.0,
    "stage_12_target_ph": 6.0,
    "stage_12_ph_tolerance": 0.2,
    "stage_12_target_ec": 2.8,
    ...
    "stage_13_trigger_hours": 288.0,
    "stage_13_target_ph": 6.0,
    "stage_13_ph_tolerance": 0.2,
    "stage_13_target_ec": 2.6,
    ...
    "stage_14_trigger_hours": 312.0,
    "stage_14_target_ph": 6.2,
    "stage_14_ph_tolerance": 0.2,
    "stage_14_target_ec": 2.4,
                    ...
                    if len(parts) >= 10:
                        ...
                        if (event_type == "PRGSample" and 
                            ...
                            try:
                                ...
                                cycle_id = parts[0].strip() # GARANTIR QUE cycle_id É EXTRAÍDO
                                
                                # Armazenar os dados do sensor
                                self.latest_sensor_data = {
                                    'time_ms': time.time() * 1000,
                                    'ph_val': ph_val,
                                    'ec_mScm': ec_val, # Nota: parts[7] é EC em ppm no V8p18. Se for mS/cm, está ok.
                                    'ph_v': float(parts[8]),  # pH (V)
                                    'ec_v': float(parts[6])   # EC (V)
                                }
                                
                                logger.info(f"Updated sensor data from event ({action}): pH={ph_val}, EC={ec_val} ppm") # Log existente comentado
                                
                                # RESETAR O PONTEIRO DE LEITURA PARA A POSIÇÃO ANTERIOR
                ...
                for line in content.split('\n'):
                    ...
                    if len(fields) >= 3:
                        ...
                        if event_type == "PRGSample":
                            ...
                            # logger.debug(f"Added event to buffer: {event_type} #{event_number} for cycle {cycle_number}, ID: {event_id}") # V8p4 não tem buffer log
                        ...
                        if event_type == "PRGSample" and event_number == "6":
                            ...
                            logger.info(f"'PRGSample' with event number 6 detected for cycle {cycle_number} - ready for dosing (ID: {event_id})") # V8p4 tem log similar, mas sem ID
                            # Mudar para:
                            # logger.info(f"'PRGSample' with event number 6 detected for cycle {cycle_number} - ready for dosing")
                            # OU, para ser mais fiel ao V8p4 que não tem ID de evento:
                            # logger.info(f"'PRGSample' with event number 6 detected for cycle {cycle_number}") # V8p4 loga "EVENT: PRGSample event #6 detected"
                            # logger.info(f"[CYCLE_SYNC] PRGSample Event #6 belongs to cycle {cycle_number} with ID {event_id}") # Comentar, V8p4 não tem
                            return True
    ...
    def chemistry_based_dosing_decision(self, measurement_type):
        ...
        if ec_continuous_monitoring and ec_deviation > ec_tolerance:
            if ec_mScm < target_ec:
                ...
                self.config['target_ec'] = target_ec  # Temporarily override
                
                _, volume_needed = self.calculate_chemical_dosing_volume("ec_plus")
                
                self.config['target_ec'] = original_target  # Restore original
                
                # Apply stage-based safety factors
                ec_dosing_count = len([e for e in self.dosing_history if e.get('pump') == 'ec_plus'])
                
                if ec_dosing_count == 0:
                    volume_needed /= 10
                    logger.info(f"First EC+ dosing in Stage {stage_config['stage_number']}: dividing volume by 10: {volume_needed:.2f} mL")
                else:
                    volume_needed /= 1
                    logger.info(f"Subsequent EC+ dosing in Stage {stage_config['stage_number']}: dividing volume by 1: {volume_needed:.2f} mL")
                ...
                if dosing_time >= self.config['min_dosing_time_ms']:
                    logger.info(f"Stage {stage_config['stage_number']} chemistry decision: Increase EC with {volume_needed:.2f} mL ({dosing_time} ms)")
