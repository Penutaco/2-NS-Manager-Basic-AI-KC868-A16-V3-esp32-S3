...
class DosingController:
    ...
    def read_latest_sensor_data(self):
        ...
        if self.check_for_stale_data():
            ...
            if self.check_for_new_files():
                ...
                # Instead of resetting to 0, synchronize with the pH pointer to avoid re‑reading old data
                self.last_read_position = self.last_ph_read_position
        ...
        try:
            ...
            # IMPORTANT: Synchronize the read pointer with the pH pointer
            self.last_read_position = self.last_ph_read_position
            ...
            with open(self.current_data_file, 'r') as file:
                ...
                # Synchronize both pointers so they remain in lock-step
                self.last_ph_read_position = self.last_read_position
            ...
            if valid_lines:
                self.latest_sensor_data = valid_lines[-1]  # Most recent valid reading
        ...
        try:
            with open(self.current_data_file, 'r') as file:
                ...
                if file_size <= self.last_ph_read_position:
                    # Se o arquivo não tiver crescido em 5 minutos, resete o ponteiro
                    current_time = time.time()
                    if not hasattr(self, 'last_file_growth_time') or (current_time - self.last_file_growth_time > 300):
                        logger.warning("Arquivo não cresceu nos últimos 5 minutos, resetando ponteiro")
                        self.last_ph_read_position = 0
                    return None
                self.last_file_growth_time = time.time()
                file.seek(self.last_ph_read_position)
                content = file.read()
                self.last_ph_read_position = file.tell()

                # Implement periodic reset check (every 10 minutes)
                current_time = time.time()
                if not hasattr(self, 'last_pointer_reset_time') or (current_time - self.last_pointer_reset_time > 600):
                    logger.info("Realizando reset periódico dos ponteiros de arquivo")
                    self.last_ph_read_position = 0
                    self.last_pointer_reset_time = current_time
    ...
    def check_for_prgsample_event2(self):
        """
        Detect trigger based on CSV row where 'Event Type' is "PRGSample" and 'Event' is 5.
            ...
            with open(self.current_event_file, 'r') as file:
                ...
                for line in content.split('\n'):
                    ...
                    if len(fields) >= 3:
                        ...
                        if event_type == "PRGSample" and event_number == "5":
                            logger.info("'PRGSample' with event number 5 detected - ready for dosing")
                            return True
                return False
        except Exception as e:
        ...
        try:
            ...
            with open(self.current_data_file, 'r') as file:
                ...
                if "PRGCal" in content:
                    ...
                    return "calibration"
                # Aqui, substituímos "PRGCal End" por "PRGSample" para retomar a operação normal
                elif "PRGSample" in content:
                    self.in_prgcal = False
                    logger.info("PRGSample detected - resuming normal dosing operation")
                    return None
        ...
        while self.running:
            try:
                ...
                if measurement_result:
                    ...
                    # Force update of sensor data to capture the latest reading
                    if self.read_latest_sensor_data():
                        logger.info("Sensor data refreshed before dosing decision")
