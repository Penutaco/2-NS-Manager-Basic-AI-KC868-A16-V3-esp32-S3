...
csv_writer.writerow([
    ...
    'EC (V)', 'EC (ppm)', 'pH (V)', 'pH Value',
    'Nitrate (V)', 'Nitrate (ppm)', 'Temp(°C)', 'Event Info'
])

# Initialize event CSV with base headers
event_csv_writer.writerow(['Event Type', 'Event Number', 'Event Time',
                           'EC (V)', 'EC (ppm)', 'pH (V)', 'pH Value', 
...
def read_data_from_platformio():
    ...
    global current_event_type, current_event_num, event_measurements
    ...
    try:
        ...
        while True:
            ...
            if line:
                # Check if this line indicates a new event
                prgcal_match = re.search(r'PRGcal Event #(\d+) activated', line)
                prgsample_match = re.search(r'PRGSample Event #(\d+) activated', line)
                
                if prgcal_match:
                    event_num = int(prgcal_match.group(1))
                    current_event_type = "PRGCal"
                    current_event_num = event_num
                    print(f"Detected PRGCal event #{event_num}")
                    
                elif prgsample_match:
                    event_num = int(prgsample_match.group(1))
                    current_event_type = "PRGSample"
                    current_event_num = event_num
                    print(f"Detected PRGSample event #{event_num}")
                ...
                if len(parts) >= 11:  # Sensor data line with 11 values
                    try:
                        ...
                        if current_event_type and current_event_num is not None:
                            ...
                            # Extract the 6 sensor values we care about: EC V, EC ppm, pH V, pH val, Nitrate V, Nitrate val
                    ...
                    # Always check for event completion regardless of filtering
                    if "completed" in line.lower() and current_event_type and current_event_num is not None:
                        event_key = f"{current_event_type}_{current_event_num}"
                        
                        if event_key in event_measurements:
                            # Write to event-based CSV
                            ...
                            print(f"Recorded event data for {event_key}")
                        
                        # Reset current event tracking
...
# Add this function to save event data on timer
def periodic_event_data_saver():
    ...
    while True:
        ...
        # If we have any events that haven't been saved, save them now
        with lock:
            for event_key, measurements in event_measurements.items():
                if measurements:
                    ...
                    event_csv_writer.writerow([
                        event_type,
                        event_num,
                        datetime.now().strftime("%H:%M:%S") + " (periodic)",
                        *measurements
                    ])
...
# Add a function to periodically rotate log files:
...
def rotate_log_files():
    ...
    while True:
        ...
        print(f"Rotated log files to {csv_filename} and {event_csv_filename}")
...
app.layout = html.Div([
    ...
    create_graph_with_controls('graph-ec-ppm', 'EC (ppm)'),
...
def update_graphs(n_intervals, time_window, viz_mode, *axis_controls):
    ...
    titles = ['Photoresistor (V)', 'Absorbance (a.u.)', 'Concentration (ppm)', 
              'EC Voltage (V)', 'EC (ppm)', 'pH Voltage (V)', 'pH Value', 
...
# ESTA É A PARTE ESSENCIAL QUE ESTAVA FALTANDO
