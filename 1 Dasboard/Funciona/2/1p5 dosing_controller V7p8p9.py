...
class DosingController:
    ...
    def detect_prgsample_ph_measurement(self):
        ...
        Returns a tuple of (measurement_type, cycle_id) if detected, None otherwise.
        """
        if not self.current_data_file:
            return None
        ...
        try:
            ...
            with open(self.current_data_file, 'r') as file:
                ...
                # Get the event info column index
                ...
                # Look for PRGSample pH measurement event
                lines = content.split('\n')
                found_data = False  # Track if we found valid data
                
                for i, line in enumerate(lines):
                    # Check for the event in the event info column or in the line as a whole
                    if "PRGSample pH measurement" in line:
                        logger.info("Detected PRGSample pH measurement event")
                        
                        # Look at data lines after the event
                        for j in range(i+1, min(i+10, len(lines))):
                            ...
                            if len(parts) >= 8:
                                try:
                                    # Extract pH from column 8 and EC from column 6
                                    ...
                                    # KEY FIX: Validate that values are not zero or invalid
                                    # Skip this line if pH or EC values are zero
                        ...
                        # If we didn't find valid data after the event, search more broadly
                        if not found_data:
    ...
    def check_for_prgsample_event2(self):
        """
        Detect 'PRGSample Event #5' using the full-scan method like that for
        'PRGSample pH measurement'. It reads all new content from the file starting
        at last_read_position and scans each line for the target string.
        """
        if not self.current_data_file:
            return False
        ...
        try:
            with open(self.current_data_file, 'r') as file:
                # Check file size; if no new data, return False
                file_size = os.path.getsize(self.current_data_file)
                if file_size <= self.last_read_position:
                    return False

                # Read all new lines from the last_read_position
                file.seek(self.last_read_position)
                content = file.read()
                self.last_read_position = file.tell()

                # Split new content into lines and scan each line for the event string
                lines = content.split('\n')
                for line in lines:
            ...
            try:
                ...
                if measurement_result:
                    ...
                    if dosing_result:
                        ...
                        logger.info("Waiting for 'PRGSample Event #2' to dose...")
