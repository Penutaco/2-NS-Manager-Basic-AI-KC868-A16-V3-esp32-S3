...
class DosingController:
        ...
        if ph_deviation > self.config['ph_tolerance']:
            ...
            if ph_val < self.config['target_ph']:
                ...
                if ph_plus_dosing_count == 0:
                    ...
                    # Subsequent dosing - divide by 1
                    volume_needed /= 1
                    logger.info(f"Subsequent pH+ dosing: dividing volume by 1: {volume_needed:.2f} mL")
    ...
    def run(self):
        ...
        while self.running:
            try:
                # ADICIONAR AQUI A VERIFICAÇÃO DO NOVO FICHEIRO DE EVENTOS
                if self.check_for_new_event_file():
                    ...
                    # Não é estritamente necessário um 'continue' aqui, pois as próximas chamadas
                    # a detect_prgsample_ph_measurement e check_for_prgsample_event2
                    # já usarão o self.current_event_file atualizado.
                ...
                if old_events or old_event_ids:
                    logger.debug(f"Removidos {len(old_events)} ciclos e {len(old_event_ids)} IDs de eventos antigos do rastreador") # Comentar, V8p4 não tem
                ...
                if self.in_prgcal:
                    logger.info("Skipping dosing cycle - PRGcal calibration in progress") # V8p4 tem "PRGCal calibration in progress - dosing suspended"
                    time.sleep(1)
                    continue
                
                if measurement_result:
                    measurement_type, cycle_id = measurement_result
                    logger.info(f"EVENT: {measurement_type} measurement detected - Cycle ID: {cycle_id}") # V8p4 tem "EVENT: pH measurement detected - Cycle ID: ..."
                    
                    # Adicionar verificação de ciclo mínimo
                    if int(cycle_id) < 2:
                        logger.info(f"Skipping dosing - Current cycle {cycle_id} is less than minimum required cycle (2)") # V8p4 não tem
                    ...
                    if dosing_result:
                        ...
                        logger.info(f"Waiting for 'PRGSample event #6' for cycle {cycle_id} to dose...") # Comentar, V8p4 não tem este detalhe
                        ...
                        if self.check_event_buffer_for_event6(cycle_id):  # Passar o cycle_id atual
                            event_detected = True
                            logger.info(f"'PRGSample Event #6' already found in buffer for cycle {cycle_id}, proceeding with dosing!") # Comentar, V8p4 não tem
                        else:
                            ...
                            while not event_detected and (time.time() - start_time < wait_time):
                                ...
                                if self.check_for_prgsample_event2():
                                    ...
                                    if hasattr(self, 'last_detected_event6_cycle') and self.last_detected_event6_cycle == cycle_id:
                                        ...
                                        logger.warning(f"Detected Event #6 for cycle {getattr(self, 'last_detected_event6_cycle', 'unknown')}, but waiting for cycle {cycle_id}") # Comentar, V8p4 não tem
                                
                                # Verificar no buffer novamente (pode ter sido adicionado por outra detecção)
                                if self.check_event_buffer_for_event6(cycle_id):
                                    event_detected = True
                                    logger.info(f"'PRGSample Event #6' found in buffer for cycle {cycle_id} during wait loop") # Comentar, V8p4 não tem
                                    break
                        ...
                        if event_detected and hasattr(self, 'last_event2_time'):
                            ...
                            if 60 <= cycle_duration <= 1800:
                                ...
                                logger.info(f"Recorded cycle duration: {cycle_duration:.0f} seconds") # Comentar, V8p4 não tem
                        ...
                        if not event_detected:
                            logger.warning(f"Timeout waiting for 'PRGSample Event #6' after {wait_time} seconds") # V8p4 não tem
                            time.sleep(1)
                            continue
                        ...
                        if event_detected:
                            ...
                            if self.is_event_already_processed(cycle_id, event_id):
                                logger.warning(f"Evento #6 para o ciclo {cycle_id}" + (f" com ID {event_id}" if event_id else "") + " já foi processado anteriormente. Ignorando.") # V8p4 não tem # CORRIGIDO
                                continue
                        
                        # STEP 7: Dose the calculated volume
                        logger.info(f"DOSING: Activating {pump_type} pump for {dosing_time}ms") # V8p4 tem
                        ...
                        if success:
                            ...
                            if event_id and event_id in self.event_unique_ids:
                                ...
                                # logger.info(f"Marcado evento com ID {event_id} como processado") # Detalhe interno
                            ...
                            logger.info(f"SUCCESS: {pump_type} dosing complete - Volume: {volume_needed:.2f} mL") # V8p4 tem
                            self.append_to_csv(pump_type, dosing_time)
                        else:
                            logger.error(f"FAILURE: {pump_type} dosing failed") # V8p4 tem
                    else: # if not dosing_result:
                        ...
                        self.last_cycle_ph_value = current_ph
                        self.last_cycle_ec_value = current_ec
                        logger.info(f"NO DOSING NEEDED in this cycle ({cycle_id}); cycle data updated for learning") # V8p4 tem "NO DOSING NEEDED..."
